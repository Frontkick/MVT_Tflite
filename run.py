...
class Tracker(object):
    ...
    def track(self, image):
        ...
        # print("Score map shape:", out_score_map.shape)
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        ...
        score_map_ctr = np.array(score_map_ctr)
        size_map = np.array(size_map)
        offset_map = np.array(offset_map)
        
        # print("Score map shape:", score_map_ctr.shape)
        # print("Size map shape:", size_map.shape)
        # print("Offset map shape:", offset_map.shape)

        # Flatten and find max score and index
        max_score = np.max(score_map_ctr.reshape(score_map_ctr.shape[0], -1), axis=1, keepdims=True)
        idx = np.argmax(score_map_ctr.reshape(score_map_ctr.shape[0], -1), axis=1)
        
        # Calculate x and y indices
        idx_y = idx // self.feat_sz
        idx_x = idx % self.feat_sz
        
        # Get size and offset values at index
        size = np.take_along_axis(size_map.reshape(size_map.shape[0], 2, -1), 
                                idx[:, None, None], axis=2).squeeze(-1)
        offset = np.take_along_axis(offset_map.reshape(offset_map.shape[0], 2, -1), 
                                    idx[:, None, None], axis=2).squeeze(-1)
        # Calculate bounding box
        bbox = np.concatenate([(idx_x.astype(np.float32) + offset[:, :1]) / self.feat_sz,
                            (idx_y.astype(np.float32) + offset[:, 1:]) / self.feat_sz,
...
if __name__ == "__main__":
    mvt_track = Tracker(model_path="MobileViT_Track_ep0300.tflite")
